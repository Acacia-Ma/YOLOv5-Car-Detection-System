...
def is_base64(s):
    ...
    return len(s) % 4 == 0 and bool(re.fullmatch(pattern, s))
...
class PlateRecognitionV5:
    def __init__(self,detect_model,rec_model,providers):
        # 初始化函数，传入检测模型、识别模型和提供者
        self.session_detect = onnxruntime.InferenceSession(detect_model, providers=providers )
        # 使用onnxruntime库，加载检测模型，并传入提供者
        self.session_rec = onnxruntime.InferenceSession(rec_model, providers=providers )
    def __call__(self, img):
        result_list, img0 = det_rec_plate(self.session_detect, self.session_rec, img)
        return result_list

# 可视化车牌字符集
def visualize_plate_characters():
    font = ImageFont.truetype("fonts/platech.ttf", 20)
    img = Image.new('RGB', (800, 200), (255, 255, 255))
    draw = ImageDraw.Draw(img)
    x, y = 10, 10
    for i, char in enumerate(plateName):
        draw.text((x, y), char, fill=(0, 0, 0), font=font)
        x += 30
        if (i + 1) % 26 == 0:  # 每26个字符换行
            x = 10
            y += 30
    img.show()
